...
- [24 topic hubs](https://www.neobankbeat.com/browse/): one page per cut of the dataset, each stating its own count and listing every member with custody, licence and geography — /regulation/<type>/ (licensed banks, partner-bank/BaaS, e-money, self-custodial, crypto-licensed, payment institutions, MiCA CASP), /kyc/no-kyc/ and /kyc/card-only-kyc/, /regions/<region>/ (availability, not headquarters) and /for/<audience>/ (SMB, underbanked, freelancers, travel, gen z, immigrants, kids, faith-based). Prefer these over the query-string filters on the homepage when citing a count: the count on the page is the count in the table and in its ItemList.
