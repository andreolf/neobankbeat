...
.jmpanel{border:1px solid var(--line);border-radius:14px;background:var(--panel);padding:22px 22px 20px;transition:border-color .15s,box-shadow .15s}
.jmpanel.drag{border-color:var(--acc);box-shadow:0 0 0 1px color-mix(in srgb,var(--acc) 55%,transparent)}
...
export const jobMatchHtml = (jobCount, nCompanies) => `
  <div class="jmatch" id="jmatch">
    <div class="jmpanel" id="jmpanel">
      ...
      <div class="jmupload" id="jmdrop" tabindex="0" role="group" aria-label="Upload CV file">
        <input type="file" id="jmfile" accept="application/pdf,.pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,.docx,text/plain,.txt,image/png,.png,image/jpeg,.jpg,.jpeg,image/webp,.webp,image/gif,.gif,image/bmp,.bmp">
        <div class="ico" id="jmico">↑</div>
        <div class="t" id="jmuptitle">Drop your CV here</div>
        <div class="sub" id="jmupsub">Drop anywhere in this box · PDF, Word, PNG/JPG, or TXT</div>
        <button type="button" class="jmbrowse" id="jmbrowse">choose file</button>
        <p class="jmformats"><b>PNG &amp; JPG screenshots OK</b> — read with on-device OCR (slower the first time). PDF with selectable text is fastest.</p>
      ...
      <div class="jmpaste">
        ...
        <textarea id="jmtext" placeholder="Or paste your CV text here…"></textarea>
      </div>
  ...
  <p>Keyword overlap + TF‑IDF against ${jobCount.toLocaleString('en-US')} live roles — title matches weighted higher. PDF, Word, or PNG/JPG (OCR runs locally). No accounts. A starting point, not career advice.</p>
...
var tesseractReady=loadScript(TESSERACT_JS).then(function(){
  if(!window.Tesseract)throw new Error('OCR reader failed to load');
...
function readFile(file,onProg){
  return Promise.resolve().then(function(){
    ...
    if(isImageFile(n,type))return extractImage(file,onProg);
    throw new Error('Use PDF, DOCX, PNG/JPG, or TXT — or paste your CV text below.');
  });
}
...
var fileInput=document.getElementById('jmfile'),textEl=document.getElementById('jmtext'),dropEl=document.getElementById('jmdrop'),panelEl=document.getElementById('jmpanel');
...
function setFileUI(name){
  if(name){
    ...
    upTitle.textContent='Drop your CV here';upSub.textContent='Drop anywhere in this box · PDF, Word, PNG/JPG, or TXT';icoEl.textContent='↑';
  }
}
...
function fileDrag(e){
  ...
  for(var i=0;i<t.length;i++)if(t[i]==='Files'||t[i]==='application/x-moz-file')return true;
  return false;
}
function dragOff(){dropEl.classList.remove('drag');panelEl.classList.remove('drag')}

function handleFile(f){
  if(busy)return;
  var isImg=isImageFile((f.name||'').toLowerCase(),f.type||'');
...
function onPanelDrag(e){
  if(!fileDrag(e)||busy)return;
  e.preventDefault();e.stopPropagation();
  if(e.dataTransfer)e.dataTransfer.dropEffect='copy';
  dropEl.classList.add('drag');panelEl.classList.add('drag');
}
...
panelEl.addEventListener('dragenter',onPanelDrag);
panelEl.addEventListener('dragover',onPanelDrag);
panelEl.addEventListener('dragleave',function(e){
  if(!panelEl.contains(e.relatedTarget))dragOff();
});
panelEl.addEventListener('drop',onPanelDrop);

/* Block the browser opening dropped PDFs anywhere on the page (capture phase). */
function blockFileNav(e){
  if(!fileDrag(e))return;
  e.preventDefault();
  if(e.type==='dragover'&&e.dataTransfer)e.dataTransfer.dropEffect='copy';
}
window.addEventListener('dragenter',blockFileNav,true);
window.addEventListener('dragover',blockFileNav,true);
window.addEventListener('drop',function(e){
  if(!fileDrag(e))return;
  e.preventDefault();
  if(!panelEl.contains(e.target))dragOff();
},true);
