...
${lockedChapters.map(t => `      <li>${esc(t)}</li>`).join('\n')}
    ...
    <form class="nbform" id="nbform" action="https://neobankbeat.substack.com/subscribe" method="get" target="_blank" rel="noopener">
      <input type="email" name="email" required placeholder="you@example.com" aria-label="Email address">
      <button type="submit">subscribe &amp; download →</button>
    </form>
    <p class="nbnote">free · via substack · confirm in the tab that opens — your download starts here automatically</p>
...
(function(){
  ...
  document.getElementById('nbform').addEventListener('submit',()=>{setTimeout(()=>unlock(true),800);});
