...
const COLUMNS = [
  ...
  { name: "regulation_type", get: (e) => e.regulation_type, description: "How it is authorised: own bank license, e-money institution, sponsor/partner bank, or unregulated" },
  { name: "license", get: (e) => e.license, description: "The named authorisation held, where disclosed" },
  { name: "card_network", get: (e) => e.card_network, description: "Card network behind the card, where one is issued" },
  { name: "card_type", get: (e) => e.card_type, description: "Card form: debit, prepaid, credit, virtual, crypto-settled or wallet-only" },
  { name: "services", get: (e) => list(e.services), description: "Verified money-movement capabilities, semicolon-separated: on-ramp, off-ramp, fiat-payin, fiat-payout, iban, multi-currency, virtual-cards, crypto-cards. Tags are omitted when unverified, so absence is not proof the capability is missing" },
  { name: "stablecoins", get: (e) => e.stablecoins, description: "Whether stablecoins are supported" },
  { name: "kyc", get: (e) => e.kyc, description: "Identity-verification requirement: Yes, No, or card-only (wallet needs no KYC, the card does)" },
  { name: "yield", get: (e) => e.yield, description: "Headline up-to yield on balances. Varies by region and plan and changes constantly — confirm with the issuer" },
  { name: "cashback", get: (e) => e.cashback, description: "Headline up-to cashback on card spend. Varies by region and plan and changes constantly — confirm with the issuer" },
  { name: "fx_markup", get: (e) => e.fx_markup?.markup, description: "Foreign-exchange markup on the free or standard plan, sourced and dated in the two columns that follow" },
  ...
  { name: "volume", get: (e) => e.volume?.figure, description: "Reported transaction volume where disclosed — self-disclosed, not audited" },
  ...
  { name: "ai", get: (e) => e.ai, description: "AI verified in production: underwriting, interface or agentic. Empty means none verified — marketing claims are deliberately not tagged" },
...
if (kaggleId) {
  ...
  const description = [
    ...
    "",
    "Most neobank lists are marketing roundups. This one records what decides whether your money is safe: **how each product is authorised** (`regulation_type`, `license` — own bank license vs e-money institution vs riding on a sponsor bank) and **who actually holds the money** (`custody` — a partner bank, the provider, or you). That regulatory layer is the part nobody else publishes in machine-readable form.",
    "",
    ...
    "- Coverage is deliberately of live consumer-facing products: defunct neobanks and pure BaaS/infrastructure providers are excluded, so this is not a survival-analysis dataset.",
    "- Unverified fields are left empty rather than guessed, which means absence is not evidence: a missing `services` tag is an unconfirmed capability, not a missing one.",
    "",
