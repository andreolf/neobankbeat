...
ok(countText().includes('358'),'shows 358 total ('+countText()+')');
...
ok(countText().includes('358'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('358'),'clear all restores 358');
...
ok(countText().includes('358'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 358/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('358'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 358/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('358'),'count back to 358');
...
ok(w.eval("D.length")===358,'dataset now 358');
ok(d.getElementById('st-total').textContent==='358','hero stat refreshed to 358');
...
ok(/^showing \d+ of 358/.test(countText())&&!countText().startsWith('showing 358'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('358'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 358 of 358'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
