...
ok(countText().includes('374'),'shows 374 total ('+countText()+')');
...
ok(countText().includes('374'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('374'),'clear all restores 374');
...
ok(countText().includes('374'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 374/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('374'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 374/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('374'),'count back to 374');
...
ok(w.eval("D.length")===374,'dataset now 374');
ok(d.getElementById('st-total').textContent==='374','hero stat refreshed to 374');
...
ok(/^showing \d+ of 374/.test(countText())&&!countText().startsWith('showing 374'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('374'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 374 of 374'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
