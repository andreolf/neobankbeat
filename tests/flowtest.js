...
ok(countText().includes('366'),'shows 366 total ('+countText()+')');
...
ok(countText().includes('366'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('366'),'clear all restores 366');
...
ok(countText().includes('366'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 366/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('366'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 366/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('366'),'count back to 366');
...
ok(w.eval("D.length")===366,'dataset now 366');
ok(d.getElementById('st-total').textContent==='366','hero stat refreshed to 366');
...
ok(/^showing \d+ of 366/.test(countText())&&!countText().startsWith('showing 366'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('366'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 366 of 366'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
