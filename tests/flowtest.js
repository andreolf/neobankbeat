...
ok(countText().includes('379'),'shows 379 total ('+countText()+')');
...
ok(countText().includes('379'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('379'),'clear all restores 379');
...
ok(countText().includes('379'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 379/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('379'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 379/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('379'),'count back to 379');
...
ok(w.eval("D.length")===379,'dataset now 379');
ok(d.getElementById('st-total').textContent==='379','hero stat refreshed to 379');
...
ok(/^showing \d+ of 379/.test(countText())&&!countText().startsWith('showing 379'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('379'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 379 of 379'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
...
console.log('— flow 25: no stale entity counts on evergreen surfaces');
{
  ...
  for(const f of evergreen){
    ...
    // "334 of 379 entities" states a subset and proves it knows the total, so the
