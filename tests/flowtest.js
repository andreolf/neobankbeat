...
ok(countText().includes('370'),'shows 370 total ('+countText()+')');
...
ok(countText().includes('showing 54'),'W filter → 54 ('+countText()+')');
...
ok(countText().includes('370'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('370'),'clear all restores 370');
...
ok(countText().includes('370'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 370/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('370'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 370/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('370'),'count back to 370');
...
ok(w.eval("D.length")===370,'dataset now 370');
ok(d.getElementById('st-total').textContent==='370','hero stat refreshed to 370');
...
ok(/^showing \d+ of 370/.test(countText())&&!countText().startsWith('showing 370'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('370'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 370 of 370'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
