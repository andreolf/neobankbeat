...
ok(countText().includes('367'),'shows 367 total ('+countText()+')');
...
ok(countText().includes('showing 52'),'W filter → 52 ('+countText()+')');
...
ok(countText().includes('367'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('367'),'clear all restores 367');
...
ok(countText().includes('367'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 367/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('367'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 367/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('367'),'count back to 367');
...
ok(w.eval("D.length")===367,'dataset now 367');
ok(d.getElementById('st-total').textContent==='367','hero stat refreshed to 367');
...
ok(/^showing \d+ of 367/.test(countText())&&!countText().startsWith('showing 367'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('367'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 367 of 367'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
