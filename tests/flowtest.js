...
ok(countText().includes('365'),'shows 365 total ('+countText()+')');
...
ok(countText().includes('showing 51'),'W filter → 47 ('+countText()+')');
...
ok(countText().includes('365'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('365'),'clear all restores 365');
...
ok(countText().includes('365'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 365/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('365'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 365/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('365'),'count back to 365');
...
ok(w.eval("D.length")===365,'dataset now 365');
ok(d.getElementById('st-total').textContent==='365','hero stat refreshed to 365');
...
ok(/^showing \d+ of 365/.test(countText())&&!countText().startsWith('showing 365'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('365'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 365 of 365'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
