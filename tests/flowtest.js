...
ok(countText().includes('357'),'shows 357 total ('+countText()+')');
...
ok(countText().includes('showing 47'),'W filter → 47 ('+countText()+')');
...
ok(countText().includes('357'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('357'),'clear all restores 357');
...
ok(countText().includes('357'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 357/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('357'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 357/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('357'),'count back to 357');
...
ok(w.eval("D.length")===357,'dataset now 357');
ok(d.getElementById('st-total').textContent==='357','hero stat refreshed to 357');
...
ok(/^showing \d+ of 357/.test(countText())&&!countText().startsWith('showing 357'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('357'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 357 of 357'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
