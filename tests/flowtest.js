...
ok(countText().includes('364'),'shows 364 total ('+countText()+')');
...
ok(countText().includes('showing 50'),'W filter → 47 ('+countText()+')');
...
ok(countText().includes('364'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('364'),'clear all restores 364');
...
ok(countText().includes('364'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 364/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('364'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 364/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('364'),'count back to 364');
...
ok(w.eval("D.length")===364,'dataset now 364');
ok(d.getElementById('st-total').textContent==='364','hero stat refreshed to 364');
...
ok(/^showing \d+ of 364/.test(countText())&&!countText().startsWith('showing 364'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('364'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 364 of 364'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
