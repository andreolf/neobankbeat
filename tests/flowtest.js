...
ok(countText().includes('372'),'shows 372 total ('+countText()+')');
...
ok(countText().includes('showing 55'),'W filter → 55 ('+countText()+')');
...
ok(countText().includes('372'),'directory nav resets filters ('+countText()+')');
...
ok(countText().includes('372'),'clear all restores 372');
...
ok(countText().includes('372'),'map filter toggles off');
...
ok(!countText().startsWith('showing 335')&&/^showing \d+ of 372/.test(countText()),'regulation filter applies ('+countText()+')');
ok(d.getElementById('activebar').textContent.includes('licensed bank'),'active bar shows regulation chip');
click(d.getElementById('navdir'));
ok(countText().includes('372'),'nav reset clears regulation too');
...
ok(/^showing \d{1,2} of 372/.test(countText()),'search women narrows ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('372'),'count back to 372');
...
ok(w.eval("D.length")===372,'dataset now 372');
ok(d.getElementById('st-total').textContent==='372','hero stat refreshed to 372');
...
ok(/^showing \d+ of 372/.test(countText())&&!countText().startsWith('showing 372'),'country click filters directory ('+countText()+')');
click(d.getElementById('navdir'));
...
ok(countText().includes('372'),'brand click resets to full directory ('+countText()+')');
...
click(cta);
ok(d.querySelector('#grid').style.display!=='none'&&!countText().includes('showing 372 of 372'),'CTA lands on the filtered directory ('+countText()+')');
click(d.getElementById('navdir'));
